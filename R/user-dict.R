...
import_cidian <- function(worker, path, tag = NULL) {
  ...
  if (!rlang::is_string(path) || is.na(path) || !nzchar(path)) {
    cli::cli_abort("`path` must be a non-empty file path string.")
  }
  ...
  path <- enc2utf8(path)
  if (!file.exists(path)) {
    cli::cli_abort("`path` must point to an existing input-method dictionary file.")
  }

  format <- tolower(tools::file_ext(path))
  supported_formats <- c("scel", "qcel", "qpyd", "bdict", "bcd")
  ...
  import_cidian_worker(worker$ptr, path, format, tag)
  invisible(NULL)
}
