...
test_that("keyword worker returns a named numeric vector by default", {
  ...
  result <- keywords(keyword_text, keys)
...
test_that("keyword worker can return a data frame", {
  keys <- worker(type = "keywords", topn = 3)
  result <- keywords(keyword_text, keys, format = "data.frame")

  result_df <- keywords_df(keyword_text, keys)
...
test_that("keyword worker can return the legacy format", {
  keys <- worker(type = "keywords", topn = 3)
  result <- keywords(keyword_text, keys, format = "legacy")
