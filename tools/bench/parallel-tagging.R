...
if (length(file_arg) > 0L) {
  ...
  if (is.null(ofile)) {
    ofile <- "tools/bench/parallel-tagging.R"
  }
...
token_count <- function(x) {
  if (is.list(x)) {
    return(sum(lengths(lapply(x, names))))
  }
  length(x)
}
...
run_case <- function(dataset, mode, input, reps) {
  rs_worker <- jiebaRS::worker(type = "tag")
  jr_worker <- jiebaR::worker(type = "tag")
  tagging_worker <- get("tagging_worker", envir = asNamespace("jiebaRS"))

  old_fun <- function(x) {
    x <- enc2utf8(x)
    x <- symbol_handle(x, rs_worker$config$symbol)
    lapply(x, function(text) {
      result <- tagging_worker(text, rs_worker$ptr)
      stats::setNames(result$tag, result$term)
    })
  }
  rs_fun <- if (identical(mode, "whole")) {
    function(x) jiebaRS::tagging(x, rs_worker, format = "legacy")
  } else {
    function(x) jiebaRS::tagging(x, rs_worker)
  }
  jr_fun <- if (identical(mode, "whole")) {
    function(x) jiebaR::tagging(x, jr_worker)
  } else {
    function(x) lapply(x, jiebaR::tagging, jiebar = jr_worker)
  }

  if (!identical(mode, "whole")) {
    stopifnot(identical(old_fun(input), rs_fun(input)))
  }
...
for (dataset in names(datasets)) {
  ...
  for (case in cases) {
    input <- if (identical(case$mode, "whole")) info$text else build_texts(info$pieces, case$n)
...
cat("parallel tagging benchmark (release)\n")
